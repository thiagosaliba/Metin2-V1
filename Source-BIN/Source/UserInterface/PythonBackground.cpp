...
static const std::string g_strPrivateShopSearchEffectName = "d:/ymir work/effect/etc/direction/direction_land2.mse";
#endif
...
void CPythonBackground::Update(float fCenterX, float fCenterY, float fCenterZ)
{
	...
	// Private Shop Target Effect
	{
		for (const auto& itor : m_kMap_dwTargetID_dwChrIDShopSearch)
		{
			...
			TPixelPosition kPixelPosition;
			pInstance->NEW_GetPixelPosition(&kPixelPosition);
			CreateSpecialEffect(itor.first, +kPixelPosition.x, -kPixelPosition.y, +kPixelPosition.z, g_strPrivateShopSearchEffectName.c_str());
		}
		for (auto itor = m_kMap_dwID_kReserveTargetEffectShopSearch.begin(); itor != m_kMap_dwID_kReserveTargetEffectShopSearch.end();)
		{
			...
			CreateSpecialEffect(itor->first, ilx, ily, fHeight, g_strPrivateShopSearchEffectName.c_str());
...
void CPythonBackground::CreateShopSearchTargetEffect(DWORD dwID, long lx, long ly)
{
	...
	CreateSpecialEffect(dwID, ilx, ily, fHeight, g_strPrivateShopSearchEffectName.c_str());
}
#endif
