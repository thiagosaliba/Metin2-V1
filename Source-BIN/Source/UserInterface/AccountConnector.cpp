...
bool CAccountConnector::__AuthState_RecvPhase()
{
	...
	else if (kPacketPhase.phase == PHASE_AUTH)
	{
		...
		strncpy(LoginPacket.hwid, CHWIDManager::instance().GetHWID().c_str(), HWID_MAX_NUM);
		...
		LoginPacket.bLanguage = LocaleService_GetLocaID();
...
bool CAccountConnector::__AuthState_RecvAuthSuccess()
{
	...
	if (!kAuthSuccessPacket.bResult || !kAuthSuccessPacket.bState)
	{
		if (m_poHandler)
