...
PyObject* playerPickAllCloseItems(PyObject* poSelf, PyObject* poArgs)
{
...
PyObject* playerGetItemApplyRandom(PyObject* poSelf, PyObject* poArgs)
{
	...
	BYTE byType;
	short sValue;
	CPythonPlayer::Instance().GetItemApplyRandom(Cell, iApplySlotIndex, &byType, &sValue);
...
PyObject* playerCanDetach(PyObject* poSelf, PyObject* poArgs)
{
	...
	// Acce Reverse Scroll
	if (CPythonAcce::Instance().IsCleanScroll(iScrollItemIndex) != FALSE)
	{
		if (CPythonPlayer::Instance().IsEquipmentSlot(TargetSlotIndex))
			return Py_BuildValue("i", DETACH_METIN_CANT);

		if (iTargetType != CItemData::ITEM_TYPE_COSTUME || iTargetSubType != CItemData::COSTUME_ACCE)
			return Py_BuildValue("i", DETACH_METIN_CANT);

		const TItemData* pPlayerItem = CPythonPlayer::Instance().GetItemData(TargetSlotIndex);
		if (pPlayerItem)
		{
			if (pPlayerItem->alSockets[CPythonAcce::ABSORBED_SOCKET] > 0)
	...
	if (iScrollItemIndex == 71109)
	{
		if (CPythonPlayer::Instance().IsEquipmentSlot(TargetSlotIndex))
			return Py_BuildValue("i", DETACH_METIN_CANT);

		if (iTargetType != CItemData::ITEM_TYPE_ARMOR && iTargetType != CItemData::ITEM_TYPE_WEAPON)
			return Py_BuildValue("i", DETACH_METIN_CANT);
	...
	// Detach Scroll
	// NOTE: Keep these conditions below Acce & Changelook
...
PyObject* playerWindowTypeToSlotTypee(PyObject* poSelf, PyObject* poArgs)
{
	int iWndType = 0;
...
PyObject* playerGetItemUnSealLeftTime(PyObject* poSelf, PyObject* poArgs)
{
	...
	long sealTime = pPlayerItem->soulbind;
...
PyObject* playerCanSealItem(PyObject* poSelf, PyObject* poArgs)
{
	...
	{
	case 2:
		if (!PyTuple_GetInteger(poArgs, 0, &iSealItemID))
			return Py_BuildException();
		if (!PyTuple_GetInteger(poArgs, 1, &TargetSlotIndex.cell))
			return Py_BuildException();
		break;
	...
	CItemData* pSealItemData;
	if (!CItemManager::Instance().GetItemDataPointer(iSealItemID, &pSealItemData))
		return Py_BuildException("can't find item data");
	...
	{
		if (pSealItemData->GetIndex() == CItemData::SEAL_ITEM_BINDING_VNUM)
		{
			if (!CPythonPlayer::Instance().GetItemUnbindTime(TargetSlotIndex))
				return Py_BuildValue("i", ATTACH_METIN_OK);
		}

		if (pSealItemData->GetIndex() == CItemData::SEAL_ITEM_UNBINDING_VNUM)
		{
			if (CPythonPlayer::Instance().GetItemUnbindTime(TargetSlotIndex))
				return Py_BuildValue("i", ATTACH_METIN_OK);
		}
...
PyObject* playerGetItemSealDate(PyObject* poSelf, PyObject* poArgs)
{
	...
	{
		//return Py_BuildException("Cell(%d, %d) item is null", Cell.window_type, Cell.cell);
		return Py_BuildValue("i", CItemData::SEAL_DATE_DEFAULT_TIMESTAMP);
	}
...
PyObject* playerSetAcceRefineWindowOpen(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonPlayer::Instance().SetAcceRefineWindowOpen(iType);
	return Py_BuildNone();
}
...
#if defined(ENABLE_CONQUEROR_LEVEL_DEBUG)
PyObject* playerGetConquerorLevel(PyObject* poSelf, PyObject* poArgs)
{
	DWORD dwLevel = CPythonPlayer::Instance().GetStatus(POINT_CONQUEROR_LEVEL);
	return Py_BuildValue("l", dwLevel);
}
...
PyObject* playerGetLoadingTip(PyObject* poSelf, PyObject* poArgs)
{
	...
	lMapIndex = CPythonNetworkStream::Instance().GetMapIndex();
...
PyObject* playerCanChangeLookClearItem(PyObject* poSelf, PyObject* poArgs)
{
	...
	const TItemData* c_pItemData = CPythonPlayer::Instance().GetItemData(Cell);
	if (c_pItemData)
		return Py_BuildValue("b", c_pItemData->dwTransmutationVnum != 0);
...
#ifdef ENABLE_QUICK_PICKUP
		{ "PickAllCloseItems", playerPickAllCloseItems, METH_VARARGS },
#endif
...
#if defined(ENABLE_CONQUEROR_LEVEL_DEBUG)
		{ "GetConquerorLevel", playerGetConquerorLevel, METH_VARARGS },
...
#ifdef ENABLE_SPECIAL_INVENTORY_SYSTEM
	PyModule_AddIntConstant(poModule, "KEY_SPECIAL_INVENTORY", KEY_SPECIAL_INVENTORY);
#endif
