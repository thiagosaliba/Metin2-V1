...
void CInstanceBase::AttachTextTail()
{
	...
	float fTextTailHeight = GetBaseHeight();
	IsMountingHorse() ? fTextTailHeight -= 20.0f : fTextTailHeight += 10.0f;
...
void CInstanceBase::__ClearAffects()
{
	...
	if ((IsStone()) && (GetVirtualNumber() != GUILD_DRAGONLAIR_STONE))
#else
	if (IsStone())
#endif
	{
...
#ifdef ENABLE_GUILD_DRAGONLAIR_SYSTEM
	else if ((IsStone()) && (GetVirtualNumber() != GUILD_DRAGONLAIR_STONE))
#else
	else if (IsStone())
#endif
	{
...
bool CInstanceBase::IsPossibleEmoticon()
{
	...
	if (ELTimer_GetMSec() - m_dwEmoticonTime < 1003)
	{
...
DWORD CInstanceBase::__AttachEffect(UINT eEftType)
{
	...
	{
#ifdef ENABLE_SKILL_COLOR_SYSTEM
		return m_GraphicThingInstance.AttachEffectByID(0, NULL, ms_adwCRCAffectEffect[eEftType], NULL, 1.0f, NULL, dwSkillColor);
#else
	...
	else
	{
		...
		if (0 == rstrBoneName.compare("PART_WEAPON"))
		{
			...
			{
#ifdef ENABLE_SKILL_COLOR_SYSTEM
				return m_GraphicThingInstance.AttachEffectByID(0, c_szBoneName, ms_adwCRCAffectEffect[eEftType], NULL, 1.0f, NULL, dwSkillColor);
#else
		...
		{
#ifdef ENABLE_SKILL_COLOR_SYSTEM
			return m_GraphicThingInstance.AttachEffectByID(0, rstrBoneName.c_str(), ms_adwCRCAffectEffect[eEftType], NULL, 1.0f, NULL, dwSkillColor);
#else
...
{
	g_GuildLeaderGradeNameMap.insert(std::make_pair(iIndex, c_szGradeName));
}
#endif
