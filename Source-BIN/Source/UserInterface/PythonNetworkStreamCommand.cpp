...
void CPythonNetworkStream::ServerCommand(char* c_szCommand)
{
	...
	// MYSHOP_DECO
	else if (!strcmpi(szCmd, "MyPrivShopOpen"))
	{
		...
		BYTE bCashItem = atoi(TokenVector[1].c_str());
		BYTE bTabCount = atoi(TokenVector[2].c_str());

		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "MyPrivShopOpen", Py_BuildValue("(ii)", bCashItem, bTabCount));
	}
	// END_OF_MYSHOP_DECO
#endif
	...
	else if (!strcmpi(szCmd, "cards"))
	{
		...
		else if ("icon" == TokenVector[1])
		{
			...
			DWORD active = atoi(TokenVector[2].c_str());
			PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_Cards_ShowIcon", Py_BuildValue("(i)", active));
		}
	}
#endif
