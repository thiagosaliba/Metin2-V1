...
bool CPythonNetworkStream::__RecvPlayerPinCodePacket()
{
	...
	m_bValidPIN = CharacterPinCodePacket.bValid;

	if (CharacterPinCodePacket.bValid)
	{
...
bool CPythonNetworkStream::IsVerifiedPIN(BYTE bIndex)
{
	if (bIndex >= PLAYER_PER_ACCOUNT5)
	{
		TraceError("CPythonNetworkStream::IsValidPIN - OUT OF RANGE SLOT(%d) > PLATER_PER_ACCOUNT(%d)",
			bIndex, PLAYER_PER_ACCOUNT5);
		return false;
	}

	if (m_bVerifiedPIN[bIndex] > 0)
		return true;

	return false;
}

bool CPythonNetworkStream::__RecvHWIDPacket()
{
	TPacketGCHWID GCPacket;
	if (!Recv(sizeof(GCPacket), &GCPacket))
		return false;

	bool bChangedHWID = GCPacket.bStatus == 1 ? true : false;
	m_bChangedHWID = bChangedHWID;

	for (BYTE i = 0; i < PLAYER_PER_ACCOUNT5; ++i)
	{
		m_bVerifiedPIN[i] = GCPacket.bVerifiedPIN[i];
	}
