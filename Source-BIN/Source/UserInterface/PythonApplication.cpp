...
bool LoadLocaleData(const char* localePath)
{
	...
	char szLocaleQuestList[256];
	snprintf(szLocaleQuestList, sizeof(szLocaleQuestList), "%s/locale_quest.txt", localePath); // Used for monster chat
	if (!rkNetStream.LoadLocaleQuestVnum(szLocaleQuestList))
		Tracenf("CPythonApplication - CPythonNetworkStream::LoadLocaleQuestVnum(%s)", szLocaleQuestList);
