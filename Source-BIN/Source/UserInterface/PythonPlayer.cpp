...
void CPythonPlayer::SPlayerStatus::SetPoint(UINT ePoint, long long llPoint)
{
	m_alPoint[ePoint] = llPoint ^ POINT_MAGIC_NUMBER;
}
...
void CPythonPlayer::__Update_AutoAttack()
{
	...
	else
	{
		...
		{
			//< 2019.02.20.Owsap - Fix auto attack with no arrows ( Movement bug floating )
...
void CPythonPlayer::SetStatus(DWORD dwType, long long llValue)
{
	...
	if (dwType == POINT_LEVEL)
	{
		...
		if (pkPlayer)
			pkPlayer->UpdateTextTailLevel(llValue);
	}

#if defined(ENABLE_CONQUEROR_LEVEL)
	if (dwType == POINT_CONQUEROR_LEVEL && llValue > 0)
	{
		CInstanceBase* pkPlayer = NEW_GetMainActorPtr();
		if (pkPlayer)
			pkPlayer->UpdateTextTailConquerorLevel(llValue);
	}
#endif
...
#endif
		m_playerStatus.SetPoint(dwType, llValue);
		__UpdateBattleStatus();
		break;
	default:
		m_playerStatus.SetPoint(dwType, llValue);
...
int CPythonPlayer::GetItemUnbindTime(TItemPos Cell)
{
	...
	return GetItemData(Cell)->soulbind;
}
...
void CPythonPlayer::SetItemUnbindTime(TItemPos Cell, DWORD dwUnbindSecondsLeft)
{
	...
	(const_cast <TItemData*>(GetItemData(Cell)))->soulbind = dwUnbindSecondsLeft;
}
#endif
...
void CPythonPlayer::RequestUseLocalQuickSlot(BYTE bLocalSlotIndex)
{
	...
	BYTE bRegisteredType;
	WORD wRegisteredItemPos;
	GetLocalQuickSlotData(bLocalSlotIndex, &bRegisteredType, &wRegisteredItemPos);

	switch (bRegisteredType)
	{
	case SLOT_TYPE_INVENTORY:
	{
		CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
		rkNetStream.SendItemUsePacket(TItemPos(INVENTORY, wRegisteredItemPos));
		break;
	}
	case SLOT_TYPE_SKILL:
	{
		ClickSkillSlot(wRegisteredItemPos);
		break;
	}
	case SLOT_TYPE_EMOTION:
	{
		PyCallClassMemberFunc(m_ppyGameWindow, "BINARY_ActEmotion", Py_BuildValue("(i)", wRegisteredItemPos));
		break;
	}
	}
}
...
void CPythonPlayer::SendClickItemPacket(DWORD dwIID)
{
	...
	{
#ifndef ENABLE_QUICK_PICKUP
		s_dwNextTCPTime = dwCurTime + 500; // fix 500
#endif
...
void CPythonPlayer::Clear()
{
	...
	m_isOpenAcceRefineWindow = false;
#endif
...
void CPythonPlayer::SetChangeLookItemData(TPacketGCChangeLookSet& rData)
{
...
void CPythonPlayer::DelChangeLookItemData(const BYTE c_bSlotIndex, const bool c_bClearEffect)
{
...
#if defined(WJ_ENABLE_PICKUP_ITEM_EFFECT)
	if (c_bClearEffect && rData.wCell < c_Inventory_Count)
	...
	rData.wCell = c_Inventory_Count;
}

void CPythonPlayer::SetChangeLookFreeItemData(TPacketGCChangeLookSet& rData)
{
...
{
#if defined(WJ_ENABLE_PICKUP_ITEM_EFFECT)
	if (c_bClearEffect && m_ChangeLookFreeYangItemSlot.wCell < c_Inventory_Count)
