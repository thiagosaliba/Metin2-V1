...
void CPythonShop::SortShopSearchData()
{
	...
	std::sort(vShopSearch.begin(), vShopSearch.end(), [](const ShopSearchData* a, const ShopSearchData* b)
		{
#if defined(ENABLE_CHEQUE_SYSTEM)
			return (a->item.price_cheque < b->item.price_cheque) || (a->item.price_cheque == b->item.price_cheque && a->item.price < b->item.price);
#else
...
#if defined(ENABLE_CHEQUE_SYSTEM)
			return Py_BuildValue("ssiii", pItemData->GetName(), ShopSearchData->name.c_str(), ShopSearchData->item.count, ShopSearchData->item.price, ShopSearchData->item.price_cheque);
#else
...
PyObject* shopGetPrivateShopItemApplyRandom(PyObject* poSelf, PyObject* poArgs)
{
	...
	if (iApplySlotIndex >= 0 && iApplySlotIndex < ITEM_APPLY_RANDOM_SLOT_MAX_NUM)
	{
		...
		if (ShopSearchData)
			return Py_BuildValue("ii", ShopSearchData->item.aApplyRandom[iApplySlotIndex].bType, ShopSearchData->item.aApplyRandom[iApplySlotIndex].sValue);
	}
...
PyObject* shopGetPrivateShopItemChangeLookVnum(PyObject* poSelf, PyObject* poArgs)
{
	...
	auto pShopSearchData = CPythonShop::Instance().GetShopSearchItemData(iIndex);
	if (pShopSearchData)
		return Py_BuildValue("i", pShopSearchData->item.dwTransmutationVnum);
