...
bool CPythonNetworkStream::RecvCharacterAdditionalInfo()
{
	...
	{
		const char* c_szName;
		if (IsNPCType(kNetActorData.m_bType) && kNetActorData.m_dwRace != 30000 && kNetActorData.m_bType != CActorInstance::TYPE_PET)
		{
			if (kNetActorData.m_bType == CActorInstance::TYPE_HORSE || kNetActorData.m_bType == CActorInstance::TYPE_PET_PAY)
			{
				char szName[255 + 1];
				if (CPythonNonPlayer::Instance().GetName(kNetActorData.m_dwRace, &c_szName))
					sprintf(szName, "%s - %s", chrInfoPacket.name, c_szName);
				else
					sprintf(szName, "%s", chrInfoPacket.name);
				kNetActorData.m_stName = szName;
			}
			else
			{
				if (CPythonNonPlayer::Instance().GetName(kNetActorData.m_dwRace, &c_szName))
