...
bool CEterPack::Create(CEterFileDict& rkFileDict, const char* dbname, const char* pathName, bool bReadOnly, const BYTE* iv)
{
	...
	strcat(m_indexFileName, s_strEIX);

	m_stDataFileName = dbname;
	m_stDataFileName += s_strEPK;
...
bool CEterPack::EncryptIndexFile()
{
	...
	if (!CLZO::Instance().CompressEncryptedMemory(zObj, pbData, file.Size(), COMPRESS_EIX))
	{
...
bool CEterPack::__BuildIndex(CEterFileDict& rkFileDict, bool bOverwrite)
{
	...
	else if (fourcc == CLZObject::ms_dwFourCC)
	{
		...
		if (!CLZO::Instance().Decompress(zObj, (const BYTE*)pvData, COMPRESS_EIX))
			return false;
...
bool CEterPack::Get(CMappedFile& out_file, const char* filename, LPCVOID* data)
{
	...
	*/
	...
	else if (COMPRESSED_TYPE_SECURITY == index->compressed_type)
	{
		...
		if (!CLZO::Instance().Decompress(*zObj, static_cast<const BYTE*>(*data), COMPRESS_EPK))
		{
	...
	else if (COMPRESSED_TYPE_HYBRIDCRYPT == index->compressed_type || COMPRESSED_TYPE_HYBRIDCRYPT_WITHSDB == index->compressed_type)
	{
		...
		if (!m_pCSHybridCryptPolicy->DecryptMemory(strFileName, static_cast<const BYTE*>(*data), index->data_size, *zObj))
		{
			delete zObj;
			return false;
		}
		...
		if (COMPRESSED_TYPE_HYBRIDCRYPT_WITHSDB == index->compressed_type)
		{
			...
			if (!m_pCSHybridCryptPolicy->GetSupplementaryDataBlock(strFileName, pSDBData, iSDBSize))
			{
				delete zObj;
				return false;
			}
...
bool CEterPack::Extract()
{
	...
	for (TDataPositionMap::iterator i = m_DataPositionMap.begin(); i != m_DataPositionMap.end(); ++i)
	{
		...
		{
			if (!CLZO::Instance().Decompress(zObj, (const BYTE*)data + index->data_position, COMPRESS_EPK))
			{
				printf("cannot decompress");
				return false;
			}
		...
		else if (COMPRESSED_TYPE_HYBRIDCRYPT == index->compressed_type || COMPRESSED_TYPE_HYBRIDCRYPT_WITHSDB == index->compressed_type)
		{
			...
			auto strFileName = std::string(index->filename);
			if (!m_pCSHybridCryptPolicy->DecryptMemory(strFileName, (const BYTE*)data + index->data_position, index->data_size, zObj))
				return false;
...
bool CEterPack::Put(const char* filename, const char* sourceFilename, BYTE packType, const std::string& strRelateMapName)
{
	...
	if (packType == COMPRESSED_TYPE_HYBRIDCRYPT || packType == COMPRESSED_TYPE_HYBRIDCRYPT_WITHSDB)
	{
		...
		auto strFileName = std::string(filename);
		m_pCSHybridCryptPolicy->GenerateCryptKey(strFileName);

		if (packType == COMPRESSED_TYPE_HYBRIDCRYPT_WITHSDB)
		{
			if (!m_pCSHybridCryptPolicy->GenerateSupplementaryDataBlock(strFileName, strRelateMapName, (const BYTE*)data, mapFile.Size(), pMappedData, iMappedDataSize))
			{
				return false;
			}
		}
...
bool CEterPack::Put(const char* filename, LPCVOID data, long len, BYTE packType)
{
		...
		packType == COMPRESSED_TYPE_COMPRESS)
	{
		if (packType == COMPRESSED_TYPE_SECURITY)
		{
			if (!CLZO::Instance().CompressEncryptedMemory(zObj, data, len, COMPRESS_EPK))
			{
				return false;
			}
		}
	...
	else if (packType == COMPRESSED_TYPE_HYBRIDCRYPT || packType == COMPRESSED_TYPE_HYBRIDCRYPT_WITHSDB)
	{
		...
		auto strFileName = std::string(filename);
		if (!m_pCSHybridCryptPolicy->EncryptMemory(strFileName, (const BYTE*)data, len, zObj))
		{
			return false;
		}
