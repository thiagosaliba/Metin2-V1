#ifndef __INC_ETER_BASE_STL_H__
#define __INC_ETER_BASE_STL_H__
...
namespace std
{
	...
	template<class _R, class _Ty, class _A> inline void_mem_fun1_t<_R, _Ty, _A> void_mem_fun1(_R(_Ty::* _Pm)(_A))
	{
...
{
	for (auto /*TContainer::iterator*/ i = container.begin(); i != container.end(); ++i)
	{
		delete* i;
		*i = NULL;
	}
...
void DeleteVectorItem(std::vector<T>* pVector, unsigned long dwIndex)
{
	...
	auto /*std::vector<T>::iterator*/ itor = pVector->begin();
...
void DeleteVectorItem(T* pVector, unsigned long dwStartIndex, unsigned long dwEndIndex)
{
	...
	auto /*T::iterator*/ itorStart = pVector->begin();
	for (unsigned long i = 0; i < dwStartIndex; ++i)
		++itorStart;

	auto /*T::iterator*/ itorEnd = pVector->begin();
...
{
	auto /*std::list<T>::iterator*/ itor = pList->begin();
...
typedef std::map<std::string, int> CMapNameToIndex;
