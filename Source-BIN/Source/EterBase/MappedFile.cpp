...
bool CMappedFile::Open(const wchar_t* filename, EFileMode mode)
{
	LPCVOID pData;
	std::wstring wSt(filename);
	std::string stFileName(wSt.begin(), wSt.end());
	return Create(stFileName.c_str(), &pData, 0, 0);
}

bool CMappedFile::Open(const char* filename, EFileMode mode)
{
	LPCVOID pData;
	if (!CEterPackManager::Instance().Get(*this, filename, &pData))
		return false;
	return true;
}
...
LPCVOID CMappedFile::GetDataPtr()
{
	return m_lpData;
}
