#ifndef __INC_ETER_BASE_UTILS_H__
#define __INC_ETER_BASE_UTILS_H__
...
void string_join(const std::string& sep, const C& container, std::string* ret)
{
	...
	{
		for (auto /*C::const_iterator*/ i = container.begin(); i != container.end(); ++i)
			capacity += (*i).length();
	}
	...
	// join strings
	{
		auto /*C::const_iterator*/ cur = container.begin();
		auto /*C::const_iterator*/ end = container.end();
		--end;
...
template <typename T>
std::string string_replace_all(T& str, const std::string& remove, const std::string& insert)
{
	std::string::size_type n = 0;
	while ((n = str.find(remove, n)) != std::string::npos)
	{
		str.replace(n, remove.size(), insert);
		n += insert.size();
	}
	return str;
}
...
#endif // __INC_ETER_BASE_UTILS_H__
