...
bool CLZObject::Decompress(uint16_t uiKeySet)
{
	UINT uiSize;
	int r;
	...
	if (m_pHeader->dwEncryptSize)
	{
		...
		__Decrypt(uiKeySet, pbDecryptedBuffer);
...
bool CLZObject::Encrypt(uint16_t uiKeySet)
{
	if (!m_bCompressed)
	{
	...
	m_pHeader->dwEncryptSize = tea_encrypt((DWORD*)pbBuffer, (const DWORD*)pbBuffer, uiKeySet, m_pHeader->dwCompressedSize + 19);
	return true;
}

bool CLZObject::__Decrypt(uint16_t uiKeySet, BYTE* pbData)
{
	assert(m_pbBuffer);

	tea_decrypt((DWORD*)pbData, (const DWORD*)(m_pbIn - sizeof(DWORD)), uiKeySet, m_pHeader->dwEncryptSize);
	return true;
}
...
bool CLZO::CompressEncryptedMemory(CLZObject& rObj, const void* pIn, UINT uiInLen, uint16_t uiKeySet)
{
	rObj.BeginCompress(pIn, uiInLen);

	if (rObj.Compress())
	{
		if (rObj.Encrypt(uiKeySet))
			return true;
...
bool CLZO::Decompress(CLZObject& rObj, const BYTE* pbBuf, uint16_t uiKeySet)
{
	if (!rObj.BeginDecompress(pbBuf))
		return false;

	if (!rObj.Decompress(uiKeySet))
		return false;
