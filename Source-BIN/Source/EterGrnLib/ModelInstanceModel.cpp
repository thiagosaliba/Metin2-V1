...
{
	granny_world_pose* grnWorldPosePtr = __GetWorldPosePtr();
	if (!grnWorldPosePtr)
	{
		granny_model* pModel = (m_pModel != NULL ? m_pModel->GetGrannyModelPointer() : NULL);
		return (pModel != NULL ? (const float*)pModel->Skeleton->Bones : NULL);
	}
