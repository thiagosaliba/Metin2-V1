...
BOOL CActorInstance::IsActTargetEmotion()
{
	DWORD dwCurMotionIndex = __GetCurrentMotionIndex();
	switch (dwCurMotionIndex)
	{
...
BOOL CActorInstance::IsActEmotion()
{
	...
	if (dwCurMotionIndex >= CRaceMotionData::NAME_CLAP && dwCurMotionIndex <= CRaceMotionData::NAME_SELFIE)
#else
	if (dwCurMotionIndex >= CRaceMotionData::NAME_CLAP && dwCurMotionIndex <= CRaceMotionData::NAME_JOY)
#endif
		return TRUE;
...
bool CActorInstance::__CanPushDestActor(CActorInstance& rkActorDst)
{
	...
	// 2020.07.16 - Disable MOB_RANK_BOSS hit back
	const CPythonNonPlayer::TMobTable* pMobTable = CPythonNonPlayer::instance().GetTable(rkActorDst.GetRace());
	if (pMobTable && pMobTable->bRank >= CPythonNonPlayer::MOB_RANK_BOSS)
		return false;
...
float CActorInstance::__GetInvisibleTimeAdjust(const UINT uiSkill, const NRaceData::TAttackData& c_rAttackData)
{
	...
	const double c_dScale = (m_fAtkSpd - 1.3) / 1.3;
	const double c_dInvTime = c_rAttackData.fInvisibleTime * 0.5;
