...
DWORD CActorInstance::AttachEffectByID(DWORD dwParentPartIndex, const char* c_pszBoneName, DWORD dwEffectID, const D3DXVECTOR3* c_pv3Position, float fParticleScale, const D3DXVECTOR3* c_pv3MeshScale, DWORD* dwSkillColor)
#else
...
#endif
{
	...
	if (c_pv3MeshScale)
#ifdef ENABLE_SKILL_COLOR_SYSTEM
		rkEftMgr.CreateEffectInstance(ae.dwEffectIndex, dwEffectID, fParticleScale, c_pv3MeshScale, dwSkillColor);
#else
		rkEftMgr.CreateEffectInstance(ae.dwEffectIndex, dwEffectID, fParticleScale, c_pv3MeshScale);
#endif
	else
#ifdef ENABLE_SKILL_COLOR_SYSTEM
		rkEftMgr.CreateEffectInstance(ae.dwEffectIndex, dwEffectID, 1.0f, NULL, dwSkillColor);
#else
