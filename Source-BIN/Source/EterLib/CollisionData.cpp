...
D3DXVECTOR3 CPlaneCollisionInstance::OnGetCollisionMovementAdjust(const CDynamicSphereInstance& s) const
{
	...
	*/
	...
	float t = -D3DXVec3Dot(&m_attribute.v3Normal, &(s.v3Position + radius_adjust - m_attribute.v3Position)) / d;

	D3DXVECTOR3 nextposition;
	nextposition = s.v3Position + t * slide;
	Tracef("$T %f", t);
	if (D3DXVec3Dot(&m_attribute.v3Normal, &advance) >= 0)
		return (t * slide - m_attribute.v3Normal * s.fRadius);
	else
		return (t * slide + m_attribute.v3Normal * s.fRadius);
	if (D3DXVec3Dot(&m_attribute.v3Normal, &advance) >= 0)
		return (t * slide + m_attribute.v3Normal * D3DXVec3Dot(&m_attribute.v3Normal, &(s.v3LastPosition - m_attribute.v3Position))) * gc_fReduceMove;
	else
		return (t * slide + m_attribute.v3Normal * D3DXVec3Dot(&m_attribute.v3Normal, &(s.v3LastPosition - m_attribute.v3Position))) * gc_fReduceMove;
	*/
}
