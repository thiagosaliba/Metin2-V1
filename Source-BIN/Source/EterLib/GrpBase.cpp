...
LPDIRECT3DVERTEXBUFFER8 CGraphicBase::ms_lpd3dTextLinePDTVB;
#endif
...
#if defined(__VERTEX_BUFFER_PERFORMANCE__)
bool CGraphicBase::SetPDTTextLineStream(SPDTVertexRaw* pSrcVertices, uint32_t uvtxCount)
{
	if (!uvtxCount)
		return false;
	...
	if (FAILED(ms_lpd3dTextLinePDTVB->Lock(0, sizeof(TPDTVertex) * uvtxCount, (uint8_t**)&pDstVertices, D3DLOCK_DISCARD)))
	{
	...
	ms_lpd3dTextLinePDTVB->Unlock();

	STATEMANAGER.SetStreamSource(0, ms_lpd3dTextLinePDTVB, sizeof(TPDTVertex));
