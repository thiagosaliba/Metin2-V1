...
void CIME::PasteTextFromClipBoard()
{
	...
	// YMD 2020.11.15.Owsap - Blocked the insertion of text tags from the clipboard.
	{
		std::vector<std::string> vec_strTag = { "|c", "|Hitem:", "|H", "|h", "|r", "|I", "|i" }; // @ TextTag.cpp
		for (std::vector<std::string>::const_iterator it = vec_strTag.begin(); it != vec_strTag.end(); it++)
		{
			std::string strFilter = *it;
			size_t pos = std::string::npos;
			while ((pos = strClipboard.find(strFilter)) != std::string::npos)
			{
				strClipboard.erase(pos, strFilter.length());
			}
		}
	}
