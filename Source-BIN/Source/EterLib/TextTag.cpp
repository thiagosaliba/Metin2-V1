...
int GetTextTag(const wchar_t* src, int maxLen, int& tagLen, std::wstring& extraInfo)
{
	...
	else if (*cur == L'I') // image |Ipath/img|i
	{
		tagLen = 2;
		return TEXT_TAG_IMAGE_START;
	}
	else if (*cur == L'i') // end of image
	{
		tagLen = 2;
		return TEXT_TAG_IMAGE_END;
	}
#endif
...
std::wstring GetTextTagOutputString(const wchar_t* src, int src_len)
{
	...
	for (int i = 0; i < src_len;)
	{
		...
		if (tag == TEXT_TAG_PLAIN || tag == TEXT_TAG_TAG)
		{
			...
			hyperlinkStep = 0;
#ifdef ENABLE_TEXT_IMAGE_LINE
		else if (tag == TEXT_TAG_IMAGE_START)
			hyperlinkStep = 1;
		else if (tag == TEXT_TAG_IMAGE_END)
			hyperlinkStep = 0;
#endif
