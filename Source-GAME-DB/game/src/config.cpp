...
int g_server_id = 0;
string g_strWebMallURL = "160.20.145.230";
...
bool g_bGlobalShout = false;
bool g_bDetailShout = false;
...
WORD gMaxItemCount = ITEM_MAX_COUNT;
...
bool g_bAcceCombineIncAbsOnLastGrade = false;
#endif

#if defined(__ANTI_EXP_RING__)
bool g_bAntiExpRing = true;
#endif
bool g_bChatWhisper = false;
...
#ifdef _AUTO_DETECT_INTERNAL_IP_
...
void config_init(const string & st_localeServiceName)
{
	...
	*log_db = '\0';
	...
	while (fgets(buf, 256, fp))
	{
		...
		TOKEN("detail_shout")
		{
			str_to_number(g_bDetailShout, value_string);
			continue;
		}
		...
		TOKEN("broadcast_name")
		{
			str_to_number(g_bBroadcastName, value_string);
			continue;
		}
		...
		TOKEN("max_item_count")
		{
			str_to_number(gMaxItemCount, value_string);
			if (gMaxItemCount > sizeof(WORD))
			{
				fprintf(stderr, "MAX_ITEM_COUNT: Overflow %d\n", sizeof(WORD));
				exit(1);
			}
			fprintf(stderr, "MAX_ITEM_COUNT: %d\n", gMaxItemCount);
			continue;
		}

		TOKEN("max_gold")
		{
			str_to_number(g_MaxGold, value_string);
			fprintf(stderr, "MAX_GOLD: %lld\n", g_MaxGold);
			continue;
		}
...
#if defined(__ACCE_COSTUME_SYSTEM__)
		TOKEN("acce_combine_inc_abs_on_last_grade")
		{
			str_to_number(g_bAcceCombineIncAbsOnLastGrade, value_string);
			continue;
		}
#endif
		...
		TOKEN("anti_exp_ring")
		{
			str_to_number(g_bAntiExpRing, value_string);
			continue;
		}
#endif
		...
		TOKEN("chat_whisper")
		{
			str_to_number(g_bChatWhisper, value_string);
			continue;
		}
		/// } OSP.CONFIGS
