...
EVENTFUNC(battle_arena_event)
{
	...
	if (pInfo != NULL)
	{
		...
		case 3:
		{
			...
			{
				pInfo->state = 6;
				SendNoticeMap("중앙 제단에 악의 기운이 모여듭니다.", pInfo->nMapIndex, false);
			}
			else
			{
				pInfo->wait_count++;
				...
				{
					pInfo->state++;
					SendNoticeMap("몬스터들이 물러갈 조짐을 보입니다.", pInfo->nMapIndex, false);
				}
				else
				{
		...
		{
			pInfo->state++;
			SendNoticeMap("몬스터들이 물러가기 시작했습니다.", pInfo->nMapIndex, false);
			SendNoticeMap("잠시 후 마을로 돌아갑니다.", pInfo->nMapIndex, false);
		...
		case 6:
		{
			...
			SendNoticeMap("몬스터들의 대장이 나타났습니다.", pInfo->nMapIndex, false);
			SendNoticeMap("30분 내로 귀목령주를 물리쳐주세요.", pInfo->nMapIndex, false);
		...
		case 7:
		{
			if (SECTREE_MANAGER::instance().GetMonsterCountInMap(pInfo->nMapIndex) <= 0)
			{
				SendNoticeMap("귀목령주와 그의 부하들을 모두 물리쳤습니다.", pInfo->nMapIndex, false);
				SendNoticeMap("잠시 후 마을로 돌아갑니다.", pInfo->nMapIndex, false);
			...
			{
				SendNoticeMap("귀목령주가 퇴각하였습니다.", pInfo->nMapIndex, false);
				SendNoticeMap("잠시 후 마을로 돌아갑니다.", pInfo->nMapIndex, false);
