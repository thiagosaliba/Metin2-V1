...
#	define IS_SUNGMA_AFFECT(type) (((type) >= AFFECT_SUNGMA_STR && (type) < AFFECT_SUNGMA_IMMUNE))
#endif
...
bool CHARACTER::UpdateAffect()
{
	...
	if (!CheckSoulItem())
	{
		...
		if (FindAffect(AFFECT_SOUL_BLUE))
			RemoveAffect(AFFECT_SOUL_BLUE);

		if (IsAffectFlag(AFF_SOUL_RED) || IsAffectFlag(AFF_SOUL_BLUE) || IsAffectFlag(AFF_SOUL_MIX))
...
#if defined(__CONQUEROR_LEVEL__)
			if (IS_SUNGMA_AFFECT(pkAff->dwType))
			{
				++it;
				continue;
			}
...
#if defined(__BLEND_AFFECT__) || defined(__EXTENDED_BLEND_AFFECT__)
			if (pkAff->dwType >= AFFECT_BLEND_POTION_1 && pkAff->dwType <= AFFECT_MOVE_SPEED)
			{
				++it;
				continue;
			}
...
#if defined(__SOUL_SYSTEM__)
			if (pkAff->dwType >= AFFECT_SOUL_RED && pkAff->dwType <= AFFECT_SOUL_MIX)
			{
				++it;
				continue;
			}
...
void CHARACTER::LoadAffect(DWORD dwCount, TPacketAffectElement* pElements)
{
	...
	if (!GetDesc()->IsPhase(PHASE_GAME))
	{
		...
		affectInfo = M2_NEW load_affect_login_event_info;
		affectInfo->pid = GetPlayerID();
		affectInfo->count = dwCount;
		affectInfo->data = M2_NEW char[sizeof(TPacketAffectElement) * dwCount];
		thecore_memcpy(affectInfo->data, pElements, sizeof(TPacketAffectElement) * dwCount);
		return;
	}
...
bool CHARACTER::AddAffect(DWORD dwType, BYTE bApplyOn, long lApplyValue, DWORD dwFlag, long lDuration, long lSPCost, bool bOverride, bool IsCube)
{
	...
	SetAffectStack(pkAff, GetAffectStack(pkAff) + 1);
...
void CHARACTER::SetAffectStack(CAffect* pkAff, BYTE value)
{
	AffectStackMap::iterator it = m_map_affectStack.find(pkAff->dwType);
...
BYTE CHARACTER::GetAffectStack(CAffect* pkAff)
{
	AffectStackMap::iterator it = m_map_affectStack.find(pkAff->dwType);
	if (it == m_map_affectStack.end())
		return 0;
