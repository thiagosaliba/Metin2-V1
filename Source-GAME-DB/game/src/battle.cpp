...
int battle_melee_attack(LPCHARACTER ch, LPCHARACTER victim)
{
	...
	{
		int max = 420; //< 2020.05.29.Owsap - Fixed short melee attack range
...
#if defined(__SHIP_DEFENSE__)
		// PC melee attack range on HUGE_RACE (Hydra)
	...
	if (pkVictim->IsNPC())
	{
...
#if defined(__ELEMENT_SYSTEM__)
		{
			if (pkVictim->GetMobElement(MOB_ELEMENT_ELECT))
				iAtk += (iAtk * (pkAttacker->GetPoint(POINT_ENCHANT_ELECT) - pkVictim->GetMobElement(MOB_ELEMENT_ELECT))) / 100;

			if (pkVictim->GetMobElement(MOB_ELEMENT_FIRE))
				iAtk += (iAtk * (pkAttacker->GetPoint(POINT_ENCHANT_FIRE) - pkVictim->GetMobElement(MOB_ELEMENT_FIRE))) / 100;

			if (pkVictim->GetMobElement(MOB_ELEMENT_ICE))
				iAtk += (iAtk * (pkAttacker->GetPoint(POINT_ENCHANT_ICE) - pkVictim->GetMobElement(MOB_ELEMENT_ICE))) / 100;

			if (pkVictim->GetMobElement(MOB_ELEMENT_WIND))
				iAtk += (iAtk * (pkAttacker->GetPoint(POINT_ENCHANT_WIND) - pkVictim->GetMobElement(MOB_ELEMENT_WIND))) / 100;

			if (pkVictim->GetMobElement(MOB_ELEMENT_EARTH))
				iAtk += (iAtk * (pkAttacker->GetPoint(POINT_ENCHANT_EARTH) - pkVictim->GetMobElement(MOB_ELEMENT_EARTH))) / 100;

			if (pkVictim->GetMobElement(MOB_ELEMENT_DARK))
				iAtk += (iAtk * (pkAttacker->GetPoint(POINT_ENCHANT_DARK) - pkVictim->GetMobElement(MOB_ELEMENT_DARK))) / 100;
...
int CalcMeleeDamage(LPCHARACTER pkAttacker, LPCHARACTER pkVictim, bool bIgnoreDefense, bool bIgnoreTargetRating)
{
	...
	if (pkAttacker->GetSungMaWill(POINT_SUNGMA_STR) > pkAttacker->GetPoint(POINT_SUNGMA_STR))
		if (iDam > 0)
			iDam /= 2;
#endif
...
void NormalAttackAffect(LPCHARACTER pkAttacker, LPCHARACTER pkVictim)
{
	...
	{
#if defined(__CONQUEROR_LEVEL__)
		BYTE bPoisonPct = pkAttacker->GetPoint(POINT_POISON_PCT);
		if (pkAttacker->GetSungMaWill(POINT_SUNGMA_IMMUNE) > pkAttacker->GetPoint(POINT_SUNGMA_IMMUNE))
			bPoisonPct /= 2;

		if (number(1, 100) <= bPoisonPct)
	...
	{
#if defined(__CONQUEROR_LEVEL__)
		BYTE bBleedPct = pkAttacker->GetPoint(POINT_BLEEDING_PCT);
		if (pkAttacker->GetSungMaWill(POINT_SUNGMA_IMMUNE) > pkAttacker->GetPoint(POINT_SUNGMA_IMMUNE))
			bBleedPct /= 2;

		if (number(1, 100) <= bBleedPct)
