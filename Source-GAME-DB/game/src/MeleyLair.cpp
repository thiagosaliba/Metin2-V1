...
namespace MeleyLair
{
	...
	EVENTFUNC(r_meleyeffect_event)
	{
		...
		if (bStep == 1)
		{
			if (bEffectStep == 1)
			{
#ifdef GUILD_DRAGONLAIR_LAZER_EFFECT_75HP
		...
		else if (bStep == 2)
		{
			if (bEffectStep == 1)
			{
#ifdef GUILD_DRAGONLAIR_LAZER_EFFECT_50HP
				time_t timeNow = static_cast<time_t>(get_dword_time());
	...
	DWORD CMgrMap::GetParticipantsCount() const
	{
		return v_Participants.size();
	}
	...
	void CMgrMap::Partecipant(bool bInsert, DWORD dwPlayerID)
	{
		if (bInsert)
		{
			bool bCheck = std::find(v_Participants.begin(), v_Participants.end(), dwPlayerID) != v_Participants.end();
			if (!bCheck)
				v_Participants.push_back(dwPlayerID);
		}
		else
			v_Participants.erase(std::remove(v_Participants.begin(), v_Participants.end(), dwPlayerID), v_Participants.end());
	}

	bool CMgrMap::IsPartecipant(DWORD dwPlayerID)
	{
		bool bCheck = false;
		if (!v_Participants.empty())
			bCheck = std::find(v_Participants.begin(), v_Participants.end(), dwPlayerID) != v_Participants.end();
	...
	LPCHARACTER CMgrMap::Spawn(DWORD dwVnum, int iX, int iY, int iDir, bool bSpawnMotion)
	{
		...
		if (pkMob)
			...
			sys_log(0, "<MeleyLair> Failed to Spawn Mob.");
	...
	void CMgrMap::Start()
	{
		if (!pkSectreeMap)
		{
			sys_log(0, "StartDungeon: no sectree");
			EndDungeon(false, true);
		}
		else
		{
			pkMainNPC = Spawn((DWORD)(NPC_VNUM), 135, 179, 8);
			pkGate = Spawn((DWORD)(GATE_VNUM), 129, 175, 5);
			pkBoss = Spawn((DWORD)(BOSS_VNUM), 130, 77, 1);
			pkStatue1 = Spawn((DWORD)(STATUE_VNUM), 123, 137, 5, true);
			pkStatue2 = Spawn((DWORD)(STATUE_VNUM), 123, 124, 5, true);
			pkStatue3 = Spawn((DWORD)(STATUE_VNUM), 136, 123, 5, true);
			pkStatue4 = Spawn((DWORD)(STATUE_VNUM), 137, 137, 5, true);
		}
	}
		...
		if (pkGuild)
		{
			...
			if (bGiveBack)
			{
				if (bSuccess)
				{
					...
					LogManager::instance().MeleyLog(GetGuildID(), GetParticipantsCount(), dwNecessaryTime);
			...
			if (pkSectreeMap)
			{
				...
				if (iWarpTime == 60)
					...
					snprintf(szBuf, sizeof(szBuf), LC_TEXT("<Guild> All the participants will be teleported out in %d seconds."), iWarpTime);
...
#ifdef GUILD_DRAGONLAIR_DESTROY_STATUE_GM
	...
	void CMgr::Destroy()
	{
		RegGuildsMap::const_iterator iter = m_RegGuilds.begin();
	...
	bool CMgr::Enter(CGuild* pkGuild, LPCHARACTER pkChar, int& iLimit)
	{
		...
		if ((pkGuild) && (pkChar))
		{
			...
			if (!pMap)
				return false;
			else if (pMap->GetParticipantsCount() >= PARTICIPANTS_LIMIT)
			{
				iLimit = 1;
				return true;
			}
	...
	{
		RegGuildsMap::const_iterator iter = m_RegGuilds.find(dwGuildID), iterEnd = m_RegGuilds.end();
		if (iter != iterEnd)
	...
	void CMgr::OpenRanking(LPCHARACTER pkChar)
	{
		...
		std::unique_ptr<SQLMsg> pMsg(DBManager::Instance().DirectQuery("SELECT * FROM log.meley_dungeon%s ORDER BY time ASC, participants ASC, date ASC LIMIT 5;", get_table_postfix()));
		if (pMsg->Get()->uiNumRows == 0)
		{
			pkChar->ChatPacket(CHAT_TYPE_COMMAND, "OpenDragonLairRanking 1");
			return;
		}
		else
		{
			pkChar->ChatPacket(CHAT_TYPE_COMMAND, "ClearDragonLairRanking 1");
			int iLine = 0;
			MYSQL_ROW mRow;
			while (NULL != (mRow = mysql_fetch_row(pMsg->Get()->pSQLResult)))
			{
				int iCur = 0;
				DWORD dwGuildID = 0, dwParticipants = 0, dwTime = 0;
				str_to_number(dwGuildID, mRow[iCur++]);
				str_to_number(dwParticipants, mRow[iCur++]);
				str_to_number(dwTime, mRow[iCur]);
				CGuild* pkGuild = CGuildManager::instance().FindGuild(dwGuildID);
				if (!pkGuild)
					continue;

				pkChar->ChatPacket(CHAT_TYPE_COMMAND, "AddDragonLairRanking 1 #%d#%s#%d#%d#", iLine, pkGuild->GetName(), dwParticipants, dwTime);
				iLine++;
			}
			pkChar->ChatPacket(CHAT_TYPE_COMMAND, "OpenDragonLairRanking 1");
		}
