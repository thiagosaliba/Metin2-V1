...
namespace quest
{
		...
		for (int i = 0; i < INVENTORY_MAX_NUM + WEAR_MAX_NUM; i++)
		{
			...
			if (pItem != NULL)
			{
				if (pItem->IsEquipped() == false)
				{
					...
					{
						lua_pushstring(L, pItem->GetLocaleName());
						lua_rawseti(L, -2, 1);
	...
	int pc_get_locale_codename(lua_State* L)
	{
	...
	{
		LPCHARACTER pkChar = CQuestManager::instance().GetCurrentCharacterPtr();
		lua_pushnumber(L, pkChar ? pkChar->GetConquerorNextExp() : 0);
		return 1;
	}
