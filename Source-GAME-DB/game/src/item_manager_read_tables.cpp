...
bool ITEM_MANAGER::GetApplyRandom(uint8_t uiIndex, uint8_t uiLevel, uint8_t& uiApplyType, int32_t& iApplyValue, uint8_t& uiPath)
{
	if (m_pApplyRandomTable == nullptr)
		return false;

	return m_pApplyRandomTable->GetApplyRandom(uiIndex, uiLevel, uiApplyType, iApplyValue, uiPath);
}

int32_t ITEM_MANAGER::GetNextApplyRandomValue(uint8_t uiIndex, uint8_t uiLevel, uint8_t uiPath, uint8_t uiApplyType)
{
	if (m_pApplyRandomTable == nullptr)
		return 0;

	return m_pApplyRandomTable->GetNextApplyRandomValue(uiIndex, uiLevel, uiPath, uiApplyType);
}
#endif
