...
EVENTFUNC(oxevent_timer)
{
	...
	{
	case 0:
		SendNoticeMap("10초뒤 판정하겠습니다.", OXEVENT_MAP_INDEX, true);
		flag++;
		return PASSES_PER_SEC(10);

	case 1:
		SendNoticeMap("정답은", OXEVENT_MAP_INDEX, true);

		if (info->answer == true)
		{
			COXEventManager::instance().CheckAnswer(true);
			SendNoticeMap("O 입니다", OXEVENT_MAP_INDEX, true);
		}
		else
		{
			COXEventManager::instance().CheckAnswer(false);
			SendNoticeMap("X 입니다", OXEVENT_MAP_INDEX, true);
		}
		...
		{
			SendNoticeMap("5초 뒤 틀리신 분들을 바깥으로 이동 시키겠습니다.", OXEVENT_MAP_INDEX, true);
		}
	...
	case 2:
		...
		SendNoticeMap("다음 문제 준비해주세요.", OXEVENT_MAP_INDEX, true);
		flag = 0;
		break;
	}
	return 0;
