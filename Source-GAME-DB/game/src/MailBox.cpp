	...
	const TItemPos& pos, const int iYang, const int iWon)
{
	...
	if (pos.IsValidItemPosition())
	{
		...
		p.AddData.ChangeLookVnum = pItem->GetTransmutationVnum();
#else
		p.AddData.ChangeLookVnum = 0;
#endif
...
/* static */ void CMailBox::Open(const LPCHARACTER ch)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("문자 메세지 전송은 레벨 20부터 할 수 있습니다."));
		return;
	}

	if (ch->PreventTradeWindow(WND_ALL))
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Please close all other windows to open your mailbox."));
		return;
	}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You must wait a while before opening your mailbox again."));
		return;
	}
...
#if defined(__MESSENGER_BLOCK_SYSTEM__)
		if (MessengerManager::instance().IsBlocked(Owner->GetName(), t->szName))
			arg = EMAILBOX_POST_WRITE::POST_WRITE_TARGET_BLOCKED;
		else if (MessengerManager::instance().IsBlocked(t->szName, Owner->GetName()))
...
bool CMailBox::GetItem(const BYTE data_idx, const bool bAll)
{
	...
	if (TotalYang + Owner->GetGold() >= g_MaxGold)
	{
		if (bAll == false)
...
#if defined(__CHANGE_LOOK_SYSTEM__)
		item->SetTransmutationVnum(mail.AddData.ChangeLookVnum);
#endif
