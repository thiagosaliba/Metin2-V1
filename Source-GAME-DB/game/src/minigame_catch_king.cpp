...
int CMiniGameManager::MiniGameCatchKing(LPCHARACTER ch, const char* data, size_t uiBytes)
{
	...
	default:
		sys_err("CMiniGameManager::MiniGameCatchKing : Unknown subheader %d : %s", p->bSubheader, ch->GetName());
		break;
	}
...
void CMiniGameManager::MiniGameCatchKingStartGame(LPCHARACTER pkChar, BYTE bSetCount)
{
	if (pkChar == NULL)
		return;
	...
	{
		pkChar->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("[Catch the King] You don't have a complete King Deck or you have insufficient Yang."));
		return;
	}
...
void CMiniGameManager::MiniGameCatchKingFieldCardClick(LPCHARACTER pkChar, BYTE bFieldPos)
{
	if (pkChar == NULL)
		return;
...
void CMiniGameManager::MiniGameCatchKingRegisterScore(LPCHARACTER pkChar, DWORD dwScore)
{
	if (pkChar == NULL)
		return;
...
int CMiniGameManager::MiniGameCatchKingGetMyScore(LPCHARACTER pkChar)
{
	if (pkChar == NULL)
		return 0;
