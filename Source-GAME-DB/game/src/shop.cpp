...
bool CShop::Create(DWORD dwVnum, DWORD dwNPCVnum, TShopItemTable* pTable)
{
	...
	*/
	...
	WORD wItemCount;
	for (wItemCount = 0; wItemCount < SHOP_HOST_ITEM_MAX_NUM; ++wItemCount)
		if (0 == (pTable + wItemCount)->vnum)
			break;

	SetShopItems(pTable, wItemCount);
	return true;
}

void CShop::SetShopItems(TShopItemTable* pTable, WORD wItemCount)
{
	if (wItemCount > SHOP_HOST_ITEM_MAX_NUM)
		return;
...
#endif
{
...
#if defined(__CHEQUE_SYSTEM__)
	if (r_item.price <= 0 && r_item.price_cheque <= 0)
#else
	if (r_item.price <= 0)
#endif
	{
	...
	uint64_t iPrice = r_item.price;
#if defined(__CHEQUE_SYSTEM__)
	uint64_t iPriceCheque = r_item.price_cheque;
#endif
	...
	{
		sys_log(1, "Shop::Buy : Not enough money : %s has %lld, price %lld", ch->GetName(), ch->GetGold(), iPrice);
		return SHOP_SUBHEADER_GC_NOT_ENOUGH_MONEY;
	}

#if defined(__CHEQUE_SYSTEM__)
	if (ch->GetCheque() < iPriceCheque)
	{
		sys_log(1, "Shop::Buy : Not enough cheque : %s has %d, price_cheque %d", ch->GetName(), ch->GetCheque(), iPriceCheque);
...
#if defined(__CHEQUE_SYSTEM__)
	ch->PointChange(POINT_CHEQUE, -iPriceCheque, false);
#endif
	...
	if (m_pkPC)
	{
...

		{
			...
			{
				snprintf(buf, sizeof(buf), "%s FROM: %u TO: %u PRICE: %llu", item->GetName(), ch->GetPlayerID(), m_pkPC->GetPlayerID(), iPrice);
			...
			snprintf(buf, sizeof(buf), "%s %u(%s) %llu %u", item->GetName(), m_pkPC->GetPlayerID(), m_pkPC->GetName(), iPrice, item->GetCount());
			LogManager::instance().ItemLog(ch, item, "SHOP_BUY", buf);

			snprintf(buf, sizeof(buf), "%s %u(%s) %llu %u", item->GetName(), ch->GetPlayerID(), ch->GetName(), iPrice, item->GetCount());
		...
		m_pkPC->PointChange(POINT_GOLD, iPrice, false);
#if defined(__CHEQUE_SYSTEM__)
		m_pkPC->PointChange(POINT_CHEQUE, iPriceCheque, false);
#endif
	...
	if (item)
		sys_log(0, "SHOP: BUY: name %s %s(x %d):%u price %u", ch->GetName(), item->GetName(), item->GetCount(), item->GetID(), iPrice);

	ch->Save();

	if (IsPCShop() && IsSoldOut())
	{
		m_pkPC->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("Your private shop sold out."));
		m_pkPC->CloseMyShop(); // CloseMyShop after ChatPacket, else the pointer will be NULL.
	}
...
bool CShop::AddGuest(LPCHARACTER ch, DWORD owner_vid, bool bOtherEmpire)
{
	...
	if (m_pkPC)
		pack2.shop_tab_count = m_pkPC->GetMyShopTabs() > 1 ? MYSHOP_MAX_TABS : 1;
	else
