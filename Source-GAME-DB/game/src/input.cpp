...
CInputProcessor::CInputProcessor() : m_pPacketInfo(NULL), m_iBufferLeft(0), m_bState(1)
{
	if (!m_pPacketInfo)
...
void CInputProcessor::Phase(const char* c_pData)
{
	if (!c_pData)
		return;
...
std::tuple<BYTE, BYTE, BYTE, BYTE, BYTE, BYTE> GetHWIDStatus(DWORD dwAID, const char* c_szHWID, bool bCheckBlock)
{
	char szHWID[HWID_MAX_NUM + 1], szOldHWID[HWID_MAX_NUM + 1] = "";
	DBManager::instance().EscapeString(szHWID, sizeof(szHWID), c_szHWID, strlen(c_szHWID));

	char szQuery[QUERY_MAX_LEN + 1];
	snprintf(szQuery, sizeof(szQuery), "SELECT `hwid`, `old_hwid`, `valid1`, `valid2`, `valid3`, `valid4`, `valid5`, `ban` FROM `account`.`hwid` WHERE `account_id` = %u", dwAID);
	std::unique_ptr<SQLMsg> pMsg(DBManager::instance().DirectQuery(szQuery));

	bool bChanged = false, bRenew = false;
	BYTE bValid[PLAYER_PER_ACCOUNT] = { 0, 0, 0, 0, 0 };

	SQLResult* pRes = pMsg->Get();
	if (pRes->uiNumRows > 0)
	{
		char szCurrentHWID[HWID_MAX_NUM + 1], szPreviousHWID[HWID_MAX_NUM + 1];
		BYTE bBanStatus = 0;

		while (MYSQL_ROW row = mysql_fetch_row(pRes->pSQLResult))
		{
			int iCount = 0;
			strlcpy(szCurrentHWID, row[iCount++], sizeof(szCurrentHWID));
			strlcpy(szPreviousHWID, row[iCount++], sizeof(szPreviousHWID));
			for (BYTE i = 0; i < PLAYER_PER_ACCOUNT; ++i)
			{
				str_to_number(bValid[i], row[iCount++]);
			}
			str_to_number(bBanStatus, row[iCount++]);

			if (bCheckBlock == true && bBanStatus > 0)
			{
				return std::make_tuple(HWID_STATUS_BLOCKED, bValid[0], bValid[1], bValid[2], bValid[3], bValid[4]);
			}

			if (strcmp(szCurrentHWID, szPreviousHWID) != 0)
			{
				strcpy(szOldHWID, szCurrentHWID);
				bChanged = true;
				break;
			}

			if (strcmp(szCurrentHWID, szHWID) != 0)
			{
				bChanged = true;
				break;
			}
	...
	{
		char szQuery[QUERY_MAX_LEN + 1];
		snprintf(szQuery, sizeof(szQuery), "REPLACE INTO `account`.`hwid` (`account_id`, `hwid`, `old_hwid`) VALUES(%u, '%s', '%s')", dwAID, szHWID, szOldHWID);
		std::unique_ptr<SQLMsg> pMsg(DBManager::instance().DirectQuery(szQuery));

		if (bChanged)
		{
			return std::make_tuple(HWID_STATUS_CHANGED, bValid[0], bValid[1], bValid[2], bValid[3], bValid[4]);
		}
	}

	return std::make_tuple(HWID_STATUS_UNTOUCHED, bValid[0], bValid[1], bValid[2], bValid[3], bValid[4]);
}
...
int CInputHandshake::Analyze(LPDESC d, BYTE bHeader, const char* c_pData)
{
	...
	else if (bHeader == HEADER_CG_PHASE)
		Phase(c_pData);
