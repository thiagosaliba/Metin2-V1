...
namespace quest
{
	...
	int monarch_bless(lua_State* L)
	{
		...
		{
			SendNoticeMap("군주의 축복으로 이지역 %s 유저는 HP,SP가 모두 채워집니다.", ch->GetMapIndex(), false,
				EMPIRE_NAME(ch->GetEmpire()));
	...
	int monarch_powerup(lua_State* L)
	{
		...
		SendNoticeMap("군주의 사자후 영향으로 이지역 %s 유저는 3분간 10 %% 의 공격력이  증가됩니다", ch->GetMapIndex(), false,
			EMPIRE_NAME(ch->GetEmpire()));
	...
	int monarch_defenseup(lua_State* L)
	{
		...
		SendNoticeMap("군주의 금강권 영향으로 이지역 %s 유저는 3분간 10 %% 의 방어력이  증가됩니다", ch->GetMapIndex(), false,
			EMPIRE_NAME(ch->GetEmpire()));
