...
void CHARACTER::ApplyMobAttribute(const TMobTable* table)
{
	...
	// NOTE: Elements of monsters don't have any counter attack against players
	// as far as I know. @Owsap
	/*
	for (int i = 0; i < MOB_ELEMENT_MAX_NUM; ++i)
	{
		if (table->cElements[i] != 0)
			ApplyPoint(aiMobElementsApplyIdx[i], table->cElements[i]);
	}
	*/
#endif
}
