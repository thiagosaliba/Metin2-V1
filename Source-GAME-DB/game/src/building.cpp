...
struct FIsIn
{
	...
	void operator () (LPENTITY ent)
	{
		...
		//< 2020.08.21.Owsap - Double check object region to avoid false positives.
		else if (ent->IsType(ENTITY_OBJECT))
		{
			if (sx <= ent->GetX() && ent->GetX() <= ex
				&& sy <= ent->GetY() && ent->GetY() <= ey)
			{
				bIn = true;
			}
		}
...
bool CLand::RequestCreateObject(DWORD dwVnum, long lMapIndex, long x, long y, float xRot, float yRot, float zRot, bool checkAnother)
{
	...
	if (checkAnother)
	{
		...
		*/
		...
		if (f.bIn)
		{
			sys_err("FIsIn another object already exist");
			return false;
		}
	}
	// END_OF_BUILDING_NPC
