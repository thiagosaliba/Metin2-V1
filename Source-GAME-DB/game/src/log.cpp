...
void LogManager::MeleyLog(DWORD dwGuildID, DWORD dwParticipants, DWORD dwTime)
{
	std::unique_ptr<SQLMsg> pMsg(DBManager::Instance().DirectQuery("SELECT participants, time FROM log.meley_dungeon%s WHERE guild_id=%u;", get_table_postfix(), dwGuildID));
	if (pMsg->Get()->uiNumRows == 0)
		Query("INSERT INTO meley_dungeon%s (guild_id, participants, time, date) VALUES(%u, %u, %u, NOW())", get_table_postfix(), dwGuildID, dwParticipants, dwTime);
	else
	{
		DWORD dwParticipantsR = 0, dwTimeR = 0;
		MYSQL_ROW mRow;
		...
		if ((dwTimeR == dwTime) && (dwParticipantsR < dwParticipants))
			Query("UPDATE meley_dungeon%s SET participants=%u, time=%u, date=NOW() WHERE guild_id=%u;", get_table_postfix(), dwParticipants, dwTime, dwGuildID);
