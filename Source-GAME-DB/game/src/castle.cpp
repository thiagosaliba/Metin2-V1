...
EVENTFUNC(castle_siege_event)
{
	...
	{
		BroadcastRestrictedNotice(LC_TEXT("%s에서 봉화를 둘러싸고 전투가 진행중입니다."), EMPIRE_NAME(GET_SIEGE_EMPIRE()));
	...
	{
		BroadcastRestrictedNotice(LC_TEXT("%s이 수성에 성공했습니다."), EMPIRE_NAME(GET_SIEGE_EMPIRE()));
		BroadcastRestrictedNotice(LC_TEXT("지금부터 %s은 30분간 봉화를 파괴하여 보상을 획득 할 수 있습니다."), EMPIRE_NAME(GET_SIEGE_EMPIRE()));
	...
	{
		//BroadcastRestrictedNotice(LC_TEXT("30분이 경과했습니다.. 봉화가 사라집니다."));
		castle_end_siege();
	}
	break;
...
bool castle_spawn_tower(int empire, int tower_count)
{
	...
	// broad cast
	{
		BroadcastRestrictedNotice(LC_TEXT("%s에 전쟁의 시작을 알리는 봉화가 나타났습니다."), EMPIRE_NAME(empire));
	}
	return true;
}
...
void castle_tower_die(LPCHARACTER ch, LPCHARACTER killer)
{
	...
	{
		int siege_end = true;
		BroadcastRestrictedNotice(LC_TEXT("%s이 봉화를 파괴했습니다."), EMPIRE_NAME(killer_empire));
		...
		if (siege_end)
		{
			...
			{
				BroadcastRestrictedNotice(LC_TEXT("%s이 수성에 실패하여 전쟁에 패배하였습니다.."), EMPIRE_NAME(GET_SIEGE_EMPIRE()));
			}
			else
			{
				BroadcastRestrictedNotice(LC_TEXT("%s이 모든 봉화를 파괴하였습니다."), EMPIRE_NAME(GET_SIEGE_EMPIRE()));
			}
			castle_end_siege();
		}
	}
