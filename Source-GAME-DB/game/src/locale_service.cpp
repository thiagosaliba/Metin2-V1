...
void LocaleService_LoadLocaleStringFile()
{
	...
	for (BYTE bLocale = LOCALE_EN; bLocale < LOCALE_MAX_NUM; ++bLocale)
	{
		g_stLocaleFilename = g_stServiceBasePath + "/country/" + GetLocaleCodeName(bLocale) + "/" + "locale_string.txt";
		if (g_stLocaleFilename.empty())
			continue;

		fprintf(stderr, "LocaleService %s\n", GetLocaleCodeName(bLocale));
		LoadLocaleString(g_stLocaleFilename.c_str(), bLocale);
	}
}

void LocaleService_LoadLocaleQuestFile()
{
	if (g_bAuthServer)
		return;
	...
	for (BYTE bLocale = LOCALE_EN; bLocale < LOCALE_MAX_NUM; ++bLocale)
	{
		g_stLocaleFilename = g_stServiceBasePath + "/country/" + GetLocaleCodeName(bLocale) + "/" + "locale_quest.txt";
		if (g_stLocaleFilename.empty())
			continue;

		LoadLocaleQuest(g_stLocaleFilename.c_str(), bLocale);
	}

	for (BYTE bLocale = LOCALE_EN; bLocale < LOCALE_MAX_NUM; ++bLocale)
	{
		g_stLocaleFilename = g_stServiceBasePath + "/country/" + GetLocaleCodeName(bLocale) + "/" + "new_locale_quest.txt";
		if (g_stLocaleFilename.empty())
			continue;
...
void LocaleService_LoadItemNameFile()
{
	...
	for (BYTE bLocale = LOCALE_EN; bLocale < LOCALE_MAX_NUM; ++bLocale)
	{
		g_stLocaleFilename = g_stServiceBasePath + "/country/" + GetLocaleCodeName(bLocale) + "/" + "item_names.txt";
		if (g_stLocaleFilename.empty())
			continue;

		LoadLocaleItemName(g_stLocaleFilename.c_str(), bLocale);
	}
}
...
void LocaleService_LoadMobNameFile()
{
	...
	for (BYTE bLocale = LOCALE_EN; bLocale < LOCALE_MAX_NUM; ++bLocale)
	{
		g_stLocaleFilename = g_stServiceBasePath + "/country/" + GetLocaleCodeName(bLocale) + "/" + "mob_names.txt";
		if (g_stLocaleFilename.empty())
			continue;

		LoadLocaleMobName(g_stLocaleFilename.c_str(), bLocale);
	}
}
...
void LocaleService_LoadSkillNameFile()
{
	...
	for (BYTE bLocale = LOCALE_EN; bLocale < LOCALE_MAX_NUM; ++bLocale)
	{
		g_stLocaleFilename = g_stServiceBasePath + "/country/" + GetLocaleCodeName(bLocale) + "/" + "skill_names.txt";
		if (g_stLocaleFilename.empty())
			continue;

		LoadLocaleSkillName(g_stLocaleFilename.c_str(), bLocale);
	}
}

void LocaleService_LoadLocaleQuizFile()
{
	if (g_bAuthServer)
		return;
	...
	for (BYTE bLocale = LOCALE_EN; bLocale < LOCALE_MAX_NUM; ++bLocale)
	{
		g_stLocaleFilename = g_stServiceBasePath + "/country/" + GetLocaleCodeName(bLocale) + "/" + "locale_quiz.txt";
		if (g_stLocaleFilename.empty())
			continue;

		LoadLocaleQuiz(g_stLocaleFilename.c_str(), bLocale);
	}
}
