...
void CPrivManager::GiveGuildPriv(DWORD guild_id, BYTE type, int value, BYTE bLog, time_t end_time_sec)
{
	...
	if (g)
	{
		if (value)
		{
			SendRestrictedNotice(LC_TEXT("%s 길드의 %s이 %d%% 증가했습니다!"), NULL, g->GetName(), GetPrivName(type), value);
		}
		else
		{
			SendRestrictedNotice(LC_TEXT("%s 길드의 %s이 정상으로 돌아왔습니다."), NULL, g->GetName(), GetPrivName(type));
		}
...
void CPrivManager::GiveEmpirePriv(BYTE empire, BYTE type, int value, BYTE bLog, time_t end_time_sec)
{
	...
	if (value)
	{
		SendRestrictedNotice(LC_TEXT("%s의 %s이 %d%% 증가했습니다!"), GetEmpireName(empire), NULL, GetPrivName(type), value);
	}
	else
	{
		SendRestrictedNotice(LC_TEXT("%s의 %s이 정상으로 돌아왔습니다."), GetEmpireName(empire), NULL, GetPrivName(type));
	}
