	...
	void operator () (LPENTITY ent)
	{
		...
		if (!pkChrVictim->Damage(m_pkChr, iDam, dt) && !pkChrVictim->IsStun())
		{
			...
			if (IS_SET(m_pkSk->dwFlag, SKILL_FLAG_SLOW | SKILL_FLAG_STUN | SKILL_FLAG_FIRE_CONT | SKILL_FLAG_POISON | SKILL_FLAG_BLEEDING))
			{
				...
				if (m_pkChr->GetSungMaWill(POINT_SUNGMA_IMMUNE) > m_pkChr->GetPoint(POINT_SUNGMA_IMMUNE))
					iPct /= 2;
#endif
...
#if defined(__SKILL_COLOR_SYSTEM__)
		if (m_dwVnum == SKILL_CHEONGRANG /* 175 */)
		{
			BYTE bBuffSlot = ESkillColorLength::BUFF_BEGIN + 5;
			BYTE bBuffID = 5;
			...
			memcpy(dwDataAttacker, m_pkAttacker->GetSkillColor(), sizeof(dwDataAttacker));
...
EVENTFUNC(skill_cheonun_event)
{
	...
	LPCHARACTER ch = info->ch;
	long lApplyValue = info->lApplyValue;

	if (ch == NULL || !lApplyValue)
		return 0;
	...
	if (lPercent <= lApplyValue)
		ch->AddAffect(2000, POINT_NONE, 0, AFF_CHUNWOON_MOOJUK, lApplyValue, 0, true);
...
void CHARACTER::SkillLearnWaitMoreTimeMessage(DWORD ms)
{
	...
	//
	if (ms < 3 * 60)
		ChatPacket(CHAT_TYPE_TALKING, LC_TEXT("몸 속이 뜨겁군. 하지만 아주 편안해. 이대로 기를 안정시키자."));
	else if (ms < 5 * 60)
		ChatPacket(CHAT_TYPE_TALKING, LC_TEXT("그래, 천천히. 좀더 천천히, 그러나 막힘 없이 빠르게!"));
	else if (ms < 10 * 60) // 10분
		ChatPacket(CHAT_TYPE_TALKING, LC_TEXT("그래, 이 느낌이야. 체내에 기가 아주 충만해."));
	else if (ms < 30 * 60) // 30분
	{
		ChatPacket(CHAT_TYPE_TALKING, LC_TEXT("다 읽었다! 이제 비급에 적혀있는 대로 전신에 기를 돌리기만 하면,"));
		ChatPacket(CHAT_TYPE_TALKING, LC_TEXT("그것으로 수련은 끝난 거야!"));
	}
	else if (ms < 1 * 3600) // 1시간
		ChatPacket(CHAT_TYPE_TALKING, LC_TEXT("이제 책의 마지막 장이야! 수련의 끝이 눈에 보이고 있어!"));
	else if (ms < 2 * 3600) // 2시간
		ChatPacket(CHAT_TYPE_TALKING, LC_TEXT("얼마 안 남았어! 조금만 더!"));
	else if (ms < 3 * 3600)
		ChatPacket(CHAT_TYPE_TALKING, LC_TEXT("좋았어! 조금만 더 읽으면 끝이다!"));
	else if (ms < 6 * 3600)
	{
		ChatPacket(CHAT_TYPE_TALKING, LC_TEXT("책장도 이제 얼마 남지 않았군."));
		ChatPacket(CHAT_TYPE_TALKING, LC_TEXT("뭔가 몸 안에 힘이 생기는 기분인 걸."));
	}
	else if (ms < 12 * 3600)
	{
		ChatPacket(CHAT_TYPE_TALKING, LC_TEXT("이제 좀 슬슬 가닥이 잡히는 것 같은데."));
		ChatPacket(CHAT_TYPE_TALKING, LC_TEXT("좋아, 이 기세로 계속 나간다!"));
	}
	else if (ms < 18 * 3600)
	{
		ChatPacket(CHAT_TYPE_TALKING, LC_TEXT("아니 어떻게 된 게 종일 읽어도 머리에 안 들어오냐."));
		ChatPacket(CHAT_TYPE_TALKING, LC_TEXT("공부하기 싫어지네."));
	}
	else //if (ms < 2 * 86400)
	{
		ChatPacket(CHAT_TYPE_TALKING, LC_TEXT("생각만큼 읽기가 쉽지가 않군. 이해도 어렵고 내용도 난해해."));
		ChatPacket(CHAT_TYPE_TALKING, LC_TEXT("이래서야 공부가 안된다구."));
	}
...
struct FComputeHealSkill
{
	FComputeHealSkill(LPCHARACTER pkHealer) : m_pkHealer(pkHealer) {}
	...
	void operator () (LPCHARACTER ch)
	{
		...
		int iRegenHP = m_pkHealer->GetMaxHP() / 100 * 15;
		int64_t iHP = (ch->GetHP() + iRegenHP >= ch->GetMaxHP()) ? ch->GetMaxHP() : ch->GetHP() + iRegenHP;
		...
		if (ch->GetHP() < ch->GetMaxHP())
		{
			...
			if (test_server)
				sys_log(0, "FComputeHealSkill: %s (pointer: %p) heal the HP of %s (pointer: %p) with %d (new HP: %d).", m_pkHealer->GetName(), get_pointer(m_pkHealer), ch->GetName(), get_pointer(ch), iRegenHP, ch->GetHP());
		}
	}
...
#if defined(__EREBUS_DUNGEON__)
	if (IsMonster() && IsHealer())
	{
		FComputeHealSkill f(this);
		if (GetParty())
			GetParty()->ForEachMemberPtr(f);
		else
			f(this);
