...
void CPrivManager::Update()
{
	...
	while (!m_pqPrivGuild.empty() && m_pqPrivGuild.top().first <= now)
	{
		...
		if (p->value != 0 && !p->bRemoved)
		{
			...
			__typeof__(m_aPrivGuild[p->type].begin()) it = m_aPrivGuild[p->type].find(p->guild_id);
	...
	while (!m_pqPrivChar.empty() && m_pqPrivChar.top().first <= now)
	{
		...
		if (!p->bRemoved)
		{
			...
			__typeof__(m_aPrivChar[p->type].begin()) it = m_aPrivChar[p->type].find(p->pid);
...
void CPrivManager::AddCharPriv(DWORD pid, BYTE type, int value)
{
	...
	__typeof__(m_aPrivChar[type].begin()) it = m_aPrivChar[type].find(pid);
...
void CPrivManager::AddGuildPriv(DWORD guild_id, BYTE type, int value, time_t duration_sec)
{
	...
	__typeof__(m_aPrivGuild[type].begin()) it = m_aPrivGuild[type].find(guild_id);
...
void CPrivManager::SendPrivOnSetup(CPeer* peer)
{
	for (int i = 1; i < MAX_PRIV_NUM; ++i)
	{
		...
		for (__typeof__(m_aPrivGuild[i].begin()) it = m_aPrivGuild[i].begin(); it != m_aPrivGuild[i].end();++it)
		{
			// ADD_GUILD_PRIV_TIME
		...
		for (__typeof__(m_aPrivChar[i].begin()) it = m_aPrivChar[i].begin(); it != m_aPrivChar[i].end(); ++it)
		{
