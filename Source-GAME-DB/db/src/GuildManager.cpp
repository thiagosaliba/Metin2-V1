...
#define for_all(cont, it) for (__typeof__((cont).begin()) it = (cont).begin(); it != (cont).end(); ++it)
...
void CGuildManager::RemoveDeclare(DWORD guild_from, DWORD guild_to)
{
	...
	__typeof__(m_DeclareMap.begin()) it = m_DeclareMap.find(TGuildDeclareInfo(0, guild_from, guild_to));
