...
class CubeWindow(ui.ScriptWindow):
		...
		if isAttached:
			...
			if app.ENABLE_EXTEND_INVEN_SYSTEM:
				...
				if app.ENABLE_SOUL_BIND_SYSTEM:
					if player.GetItemSealDate(window, attachedSlotPos) == -1 or player.GetItemSealDate(window, attachedSlotPos) > 0:
						return
	...
	def __UpdateCantMouseEvent(self):
		...
		if app.ENABLE_SPECIAL_INVENTORY_SYSTEM:
			invenSpecialPage = self.invenSpecial.GetInventoryPageIndex()
		...
		for slotPos, invenPos in self.cubeItemInfo.items():
			...
			if app.ENABLE_SPECIAL_INVENTORY_SYSTEM:
				if self.invenSpecial.GetInventoryType() == 0:
					if (item.SKILL_BOOK_INVENTORY_SLOT_START + (invenSpecialPage * player.SPECIAL_INVENTORY_PAGE_SIZE)) <= inven_slot_pos < (item.SKILL_BOOK_INVENTORY_SLOT_START + (((invenSpecialPage + 1) * player.SPECIAL_INVENTORY_PAGE_SIZE))):
						inven_slot_pos = inven_slot_pos - (item.SKILL_BOOK_INVENTORY_SLOT_START + (invenSpecialPage * player.SPECIAL_INVENTORY_PAGE_SIZE))
						self.invenSpecial.wndItem.SetCantMouseEventSlot(inven_slot_pos)

				if self.invenSpecial.GetInventoryType() == 1:
					if (item.UPGRADE_ITEMS_INVENTORY_SLOT_START + (invenSpecialPage * player.SPECIAL_INVENTORY_PAGE_SIZE)) <= inven_slot_pos < (item.UPGRADE_ITEMS_INVENTORY_SLOT_START + (((invenSpecialPage + 1) * player.SPECIAL_INVENTORY_PAGE_SIZE))):
						inven_slot_pos = inven_slot_pos - (item.UPGRADE_ITEMS_INVENTORY_SLOT_START + (invenSpecialPage * player.SPECIAL_INVENTORY_PAGE_SIZE))
						self.invenSpecial.wndItem.SetCantMouseEventSlot(inven_slot_pos)

				if self.invenSpecial.GetInventoryType() == 2:
					if (item.STONE_INVENTORY_SLOT_START + (invenSpecialPage * player.SPECIAL_INVENTORY_PAGE_SIZE)) <= inven_slot_pos < (item.STONE_INVENTORY_SLOT_START + (((invenSpecialPage + 1) * player.SPECIAL_INVENTORY_PAGE_SIZE))):
						inven_slot_pos = inven_slot_pos - (item.STONE_INVENTORY_SLOT_START + (invenSpecialPage * player.SPECIAL_INVENTORY_PAGE_SIZE))
						self.invenSpecial.wndItem.SetCantMouseEventSlot(inven_slot_pos)

				if self.invenSpecial.GetInventoryType() == 3:
					if (item.GIFT_BOX_INVENTORY_SLOT_START + (invenSpecialPage * player.SPECIAL_INVENTORY_PAGE_SIZE)) <= inven_slot_pos < (item.GIFT_BOX_INVENTORY_SLOT_START + (((invenSpecialPage + 1) * player.SPECIAL_INVENTORY_PAGE_SIZE))):
						inven_slot_pos = inven_slot_pos - (item.GIFT_BOX_INVENTORY_SLOT_START + (invenSpecialPage * player.SPECIAL_INVENTORY_PAGE_SIZE))
						self.invenSpecial.wndItem.SetCantMouseEventSlot(inven_slot_pos)
	...
	def __SetCanMouseEventSlot(self, invenPos):
		...
		if app.ENABLE_SPECIAL_INVENTORY_SYSTEM:
			if self.invenSpecial.GetInventoryType() == 0:
				if (item.SKILL_BOOK_INVENTORY_SLOT_START + (invenSpecialPage * player.SPECIAL_INVENTORY_PAGE_SIZE)) <= invenPos < (item.SKILL_BOOK_INVENTORY_SLOT_START + (((invenSpecialPage + 1) * player.SPECIAL_INVENTORY_PAGE_SIZE))):
					invenPos = invenPos - (item.SKILL_BOOK_INVENTORY_SLOT_START + (invenSpecialPage * player.SPECIAL_INVENTORY_PAGE_SIZE))
					self.invenSpecial.wndItem.SetCanMouseEventSlot(invenPos)

			if self.invenSpecial.GetInventoryType() == 1:
				if (item.UPGRADE_ITEMS_INVENTORY_SLOT_START + (invenSpecialPage * player.SPECIAL_INVENTORY_PAGE_SIZE)) <= invenPos < (item.UPGRADE_ITEMS_INVENTORY_SLOT_START + (((invenSpecialPage + 1) * player.SPECIAL_INVENTORY_PAGE_SIZE))):
					invenPos = invenPos - (item.UPGRADE_ITEMS_INVENTORY_SLOT_START + (invenSpecialPage * player.SPECIAL_INVENTORY_PAGE_SIZE))
					self.invenSpecial.wndItem.SetCanMouseEventSlot(invenPos)

			if self.invenSpecial.GetInventoryType() == 2:
				if (item.STONE_INVENTORY_SLOT_START + (invenSpecialPage * player.SPECIAL_INVENTORY_PAGE_SIZE)) <= invenPos < (item.STONE_INVENTORY_SLOT_START + (((invenSpecialPage + 1) * player.SPECIAL_INVENTORY_PAGE_SIZE))):
					invenPos = invenPos - (item.STONE_INVENTORY_SLOT_START + (invenSpecialPage * player.SPECIAL_INVENTORY_PAGE_SIZE))
					self.invenSpecial.wndItem.SetCanMouseEventSlot(invenPos)

			if self.invenSpecial.GetInventoryType() == 3:
				if (item.GIFT_BOX_INVENTORY_SLOT_START + (invenSpecialPage * player.SPECIAL_INVENTORY_PAGE_SIZE)) <= invenPos < (item.GIFT_BOX_INVENTORY_SLOT_START + (((invenSpecialPage + 1) * player.SPECIAL_INVENTORY_PAGE_SIZE))):
					invenPos = invenPos - (item.GIFT_BOX_INVENTORY_SLOT_START + (invenSpecialPage * player.SPECIAL_INVENTORY_PAGE_SIZE))
					self.invenSpecial.wndItem.SetCanMouseEventSlot(invenPos)
		...
		for slotPos, invenPos in self.cubeItemInfo.items():
			...
			if app.ENABLE_SPECIAL_INVENTORY_SYSTEM:
				if self.invenSpecial.GetInventoryType() == 0:
					if (item.SKILL_BOOK_INVENTORY_SLOT_START + (invenSpecialPage * player.SPECIAL_INVENTORY_PAGE_SIZE)) <= inven_slot_pos < (item.SKILL_BOOK_INVENTORY_SLOT_START + (((invenSpecialPage + 1) * player.SPECIAL_INVENTORY_PAGE_SIZE))):
						inven_slot_pos = inven_slot_pos - (item.SKILL_BOOK_INVENTORY_SLOT_START + (invenSpecialPage * player.SPECIAL_INVENTORY_PAGE_SIZE))
						self.invenSpecial.wndItem.SetCanMouseEventSlot(inven_slot_pos)
