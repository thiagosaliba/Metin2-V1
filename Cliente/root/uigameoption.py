...
class OptionDialog(ui.ScriptWindow):
	...
	if app.ENABLE_GUILD_DRAGONLAIR_SYSTEM:
		...
		def IsRedDragonLairMap(self, button):
			mapName = background.GetCurrentMapName()
			if mapName == "metin2_map_n_flame_dragon":
	...
	def __OnClickPvPModeRevengeButton(self):
		if app.ENABLE_GUILD_DRAGONLAIR_SYSTEM:
			if self.IsRedDragonLairMap(player.PK_MODE_REVENGE):
				return
	...
	def __OnClickPvPModeFreeButton(self):
		if app.ENABLE_GUILD_DRAGONLAIR_SYSTEM:
			if self.IsRedDragonLairMap(player.PK_MODE_FREE):
				return
	...
	def __OnClickPvPModeGuildButton(self):
		if app.ENABLE_GUILD_DRAGONLAIR_SYSTEM:
			if self.IsRedDragonLairMap(player.PK_MODE_GUILD):
				return
